...
\begin{frame}
  ...
  We introduce a bias: orientation of \( \geq 1 \) particle (the "informed" one/ "scout"). 
...
\begin{itemize}
	...
	\item \( \vec{v}^{2} \) \xrightarrow{n \to \infty} \( \alpha / \beta \) coming from \( (\alpha - \vec{v}^2 \beta ) \vec{v} = 0 \) for steady state
	\item confirmation of ring formation \\
	- \( R_{max}, R_{mean}, R_{min} \) converge to same value \\
	- \( v_{radial} \to 0 \) and \( v_{tangential} \to | v |  \) for \( N \to \infty \)
