...
def simulate(steps = 10, a = 1, dt = 0.01, randomspeeds = True):
	...
	for i in range(steps):
		...
		for i in range(len(agents)):
			...
			coordinate=Point(agent[0], agent[1])
			coordinate.draw(window)		
...
#		coordinate=Point(agent[0], agent[1])
#		coordinate.draw(window)
#		coordinate=Point(150,50)
#		(agent[0], agent[1]).draw(window)
#		coordinate.draw(window)
...
#agent[0] = Point(random.uniform(0,winWidth), random.uniform(0,winHeight))
 ...
 #       agent[0].draw(window) # HEREEEEE


#simulate()
simulate(10, 1, 0.02, True)
